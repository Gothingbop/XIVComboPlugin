...
internal struct CooldownData
{
    ...
    /// </summary>
    public unsafe float CastTime => ActionManager.GetAdjustedCastTime(ActionType.Action, ActionID) / 1000f;

    /// <summary>
    /// Gets the resource cost of the action.
    /// </summary>
    public unsafe float Cost => ActionManager.GetActionCost(ActionType.Action, ActionID, 1, 0, 0, 0);

    /// <summary>
    /// Gets the base cooldown time of an action in seconds, adjusted for spell recast modifiers
    /// (ex. spell speed, if relevant).
    /// </summary>
    public float BaseCooldown => ActionManager.GetAdjustedRecastTime(ActionType.Action, ActionID) / 1000f;

    /// <summary>
    /// Gets the total cooldown of an action across all charges, which is equivalent to the BaseCooldown multiplied
    /// by the MaxCharges.
    /// </summary>
    public float TotalBaseCooldown => BaseCooldown * MaxCharges;
    ...
    /// </summary>
    public ushort RemainingCharges => !isCooldown ? MaxCharges : (ushort)(TotalCooldownElapsed / BaseCooldown);
    ...
    /// </summary>
    public bool Available => !isCooldown || RemainingCharges > 0;
    ...
    /// </summary>
    public float TotalCooldownRemaining => !isCooldown ? 0 : TotalBaseCooldown - cooldownElapsed;
    ...
    /// </summary>
    public float CooldownRemaining => TotalCooldownRemaining % BaseCooldown;
    ...
    /// </summary>
    public float TotalCooldownElapsed => !isCooldown ? TotalBaseCooldown : cooldownElapsed;
    ...
    /// </summary>
    public float CooldownElapsed => TotalCooldownElapsed % BaseCooldown;
}
